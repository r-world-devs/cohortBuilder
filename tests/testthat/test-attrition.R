test_that("get_attrition_coords returns a data frame with expected structure and values", {
  ...
  expect_equal(nrow(result), length(labels))
  
  # Check that space attribute is retained correctly
  expect_equal(attr(result, "space"), space)
  ...
  expect_equal(result$label, expected_label)
  ...
  expect_equal(result$label_heights, c(1, 2))
  ...
  expect_equal(result$label_position_y, c(0, 2))
  ...
  expect_equal(result$arrow_end_position_y, c(2, NA_real_))
  ...
  expect_equal(result$excl_position_y, c(NA_real_, 1.5))
})

...
```{r}
S7::method(cb_get_filter_stats, list(CbFilterLogical, tblist_class)) <- function(filter, source, data_object, name, ...) {
  dataset <- filter@dataset
  variable <- filter@variable
  if (missing(name)) {
...
```{r}
S7::method(cb_get_filter_defaults, list(CbFilterLogical, tblist_class)) <- function(filter, source, data_object, cache_object, ...) {
...
unique(get_data(coh)$iris$is_setosa)
```
